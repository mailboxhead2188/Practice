...
decompressKeyCheckBuffer:	times COMMON_COMPRESS_KEY_LENGTH_V1+1 db 0
decompressKeyBuffer:		times COMMON_COMPRESS_KEY_LENGTH_V1+1 db 0
compressKeyBuffer:			times COMMON_COMPRESS_KEY_LENGTH_V1+1 db 0
...
common_compress_inflate__v1__loop:
	...
	mov ax, cs
	mov es, ax
	mov di, decompressKeyCheckBuffer		; ES:DI := temp key check buffer
	mov cx, COMMON_COMPRESS_KEY_LENGTH_V1
	rep movsb
	mov di, decompressKeyCheckBuffer		; ES:DI := temp key check buffer
	
	mov ds, ax
	mov si, decompressKeyBuffer				; DS:SI := key
	...
	cmp ax, 0
	jne common_compress_inflate__v1__loop_non_run	; key mismatch, so no run
...
_compress_find_key:
	...
	; copy source buffer into destination buffer, so we can
	; add a 0 terminator at the end, to use string operations on it
	pusha
	rep movsb
	mov byte [es:di], 0						; terminate (to treat as string)
	popa
	
	push es
	pop ds
	mov si, di					; DS:SI := ptr to 0-terminated source buffer
...
_compress_find_key__loop:
	; here, DS:SI = ptr to 0-terminated source buffer
	...
	; verify key
	call common_string_first_indexof		; AX := 0 when not found
											; BX := index when found
	cmp ax, 0
