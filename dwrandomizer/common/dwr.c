...
/**
 * Randomizes the spell names.
 * * @param rom The rom struct
 */
static void randomize_spell_names(dw_rom *rom)
{
    int i=0;
    
    if (!(RANDOMIZE_SP_NAMES(rom) && RANDOMIZE_SPELLS(rom)))
        return;

    // Randomize the sizes
    int sizes[10];
    uint8_t patchValue[72];
    int wordCount = 0;
    int wordSizeCount = 0;
    sizes[0] = rand() % (8 - 3) + 3;
    sizes[2] = rand() % (8 - 3) + 3;
    ...
    for(i; i<72; i++) {
        wordSizeCount++;
        if (wordSizeCount == sizes[wordCount]) {
            wordCount++;

            // End of buffer, fill the rest.
            if (wordCount == 10) {
                for(int j=i; j<72; j++) {
                    patchValue[j] = 0xff;
                }
        ...
        // ...
        // Z = 0x3d = 61
        patchValue[i] = rand() % (0x3d - 0x24 + 1) + 0x24;
    }
    patch(rom, 0x07e56, 72, &patchValue);
}
